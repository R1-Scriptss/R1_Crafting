...
RegisterNetEvent('r1_crafting:giveitems')
AddEventHandler('r1_crafting:giveitems', function(craftingType, craftingOption)
    local src = source
    ...
    if hasRequiredItems then
        local result = lib.callback.await('r1_crafting:serverprogress', src, text, time)
    ...
    local embeds = {
        {
            ...
            ["color"] = 56108,
            ["footer"] = {
                ["text"] = "r1_crafting " .. os.date('%H:%M - %d. %m. %Y', os.time()),
            },
        }
    }
